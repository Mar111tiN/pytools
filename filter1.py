...
def filter1(
    ...
    if "xls" in os.path.splitext(filter_file)[1]:
        filter_settings = pd.read_excel(
            filter_file, sheet_name=filter_sheet, index_col=0
        )[:4]
    else:
    ...
    def filter_basic(df, keep_syn=False):
        ...
        exon_func = (
            df["ExonicFunc"] != "unknown"
            if keep_syn
            else ~df["ExonicFunc"].isin(["unknown", "synonymous SNV"])
        )
    ...
    # filter and sort
    filter1_df = filter_1(basic_df, _filter="filter1")
